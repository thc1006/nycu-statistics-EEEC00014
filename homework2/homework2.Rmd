...
```{r ex5_41-fig-qq, echo=FALSE, out.width="80%", fig.cap="Ex 5-41 配對差值的 Normal Q-Q plot。X 軸是理論常態分位數、Y 軸是樣本分位數（單位：分鐘）；12 個點都貼著紅色參考線，配合 $W = 0.962$、$p = 0.807$ 可以接受差值的常態假設。"}
knitr::include_graphics("figures/5-41_diff_qq.png")
```

```{r ex5_41-fig-hist, echo=FALSE, out.width="80%", fig.cap="Ex 5-41 配對差值的直方圖與套疊的常態曲線。X 軸是 Coding.diff（分鐘）、Y 軸是 density；底部 rug 把 12 個觀測值的位置點出來。"}
knitr::include_graphics("figures/5-41_diff_hist.png")
```
