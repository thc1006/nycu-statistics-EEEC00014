...
依助教規定，過程數值算到第三位、最終答案四捨五入到第三位。共用設定（套件、
`fmt3`、`print_header`）放在 `Homework_Assignments_3_solution.R` 的最前段；本
PDF 只呈現每小題的統計計算程式碼與 console 結果，繪圖的 `ggplot` 樣板碼放在獨立
的 `_plot` 區塊執行但不嵌入 PDF。完整原始碼見 `.R` 檔，完整 console 輸出見
`homework_assignments_3_output.txt`。

> 校訂註記：PDF 在 6-5(d) 有 StrikeOut + Caret 插入 `the estimated`，所以 (d)
> 讀作 "Find the standard error of the **estimated** slope and intercept
> coefficients."；6-11 的 $x_0$ 是 **2.1**（PDF 文字層誤判成 `2 .1`）。
...
完整 R 原始碼見 \texttt{Homework\_Assignments\_3\_solution.R}，完整 console 輸出見
\texttt{homework\_assignments\_3\_output.txt}。
\end{flushleft}
